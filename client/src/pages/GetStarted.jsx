...
import { useCouponsStatusQuery } from "../features/api/couponApi";

const GetStarted = () => {
  const { data, isLoading } = useCouponsStatusQuery();
  ...
  return (
    <div className="flex z-10 relative flex-col mt-3 mb-24">
      <div className="px-6">
        ...
        {data?.result?.total === data?.result?.participant ? (
          <div className="mt-8">
            <p className="bg-red-500 text-white text-center border border-red-600 rounded-lg px-4 py-4">
              Sorry No Gift Available
            </p>
          </div>
        ) : (
          <div>
            ...
            <button
              onClick={handleNext}
              className="bg-blue-700 mt-7 block w-full rounded-xl text-white text-center p-4">
              Get Started
            </button>
          </div>
        )}
